...
<div>
  <div class="position-box">
    <div class="detail-content nodate" v-if="count === 0">暂无消费明细记录</div>
    <vue-better-scroll ref="scroll"
      ...
      <div class="detail-content">
        <div class="items clearfix" v-for="(detail, index) in details" :key="index">
          ...
          <div class="unit">{{detail.wareqty}}{{detail.wareunit}}</div>
        </div>
      </div>
    </vue-better-scroll>
  </div>
...
<script>
import { getConsumptionDetails } from '@/api/user'
...
export default {
  ...
  methods: {
    init () {
      loading.show()
      let self = this
      getConsumptionDetails(this.pageIndex, this.pageSize).then(res => {
        self.details = res.page.records
    ...
    onPullingDown () {
      let self = this
      self.pageIndex = 1
      getConsumptionDetails(self.pageIndex, self.pageSize).then(res => {
        self.details = res.page.records
        self.count = self.details.length
    ...
    onPullingUp () {
      ...
      getConsumptionDetails(self.pageIndex, self.pageSize).then(res => {
        ...
        self.details = self.details.concat(...res.page.records)
        self.count = self.details.length
