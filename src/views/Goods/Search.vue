...
<div>
  ...
  <header class='weui-header fixed-top'>
    <PageHeader title="商品搜索"></PageHeader>
    <div class="pro-sort">
      <div class="weui-flex">
        <div class="weui-flex__item"><div class="placeholder NormalCss">关键词:{{currKey}}</div></div>
      </div>
    </div>
  </header>
...
export default {
  ...
  methods: {
    init () {
      loading.show()
      let self = this
      self.currKey = this.$route.query.key
      Promise.all(
        [
          searchGoods(self.currKey, self.pageIndex, self.pageSize),
          getHeadquarters()
        ]
    ...
    onPullingDown () {
      let self = this
      self.pageIndex = 1
      searchGoods(self.currKey, self.pageIndex, self.pageSize).then(res => {
